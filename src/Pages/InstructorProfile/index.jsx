...
function StudentProfile() {
  ...
  const getScheduleSessions = async (id) => {
    ...
    await axios
      .get(`http://localhost:5000/scheduleSessions`, TeacherParams)
      .then((res) => {
  ...
  const handleCheckboxChange = (e) => {
    ...
    setSelectedSlots((prevSelectedSlots) => {
      if (isChecked) {
        ...
        const filteredSlots = prevSelectedSlots.filter(
          (slot) =>
            !(
              slot.jour === jour &&
      ...
      try {
        ...
        if (token) {
          const userDataResponse = await axios.get(
            "http://localhost:5000/api/userToken",
            {
              headers: {
          ...
          const disponibilitesResponse = await axios.get(
            `http://localhost:5000/disponibilte/getByUser/${userId}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
          ...
          const userCoursesResponse = await axios.get(
            `http://localhost:5000/api/${userDataResponse.data._id}/courses`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
    ...
    try {
      ...
      if (token) {
        ...
        const response = await axios.put(
          `http://localhost:5000/disponibilte/user/updateAvail/${userId}`,
          requestBody,
          {
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const handleInputChange = (event) => {
    ...
    if (name === "msg") {
      // Assuming 'msg' is the name of the date input field
                ...
                <div className="col-lg-3">
                  <div className="teacher-profile">
                    ...
                    <CardBody className="border-top p-4">
                      <div>
                        <CardSubtitle className="text-muted fs-5">
                          Email address
                        </CardSubtitle>
                        ...
                        <CardTitle tag="h4">
                          {userData.dateNaiss
                            ? format(new Date(userData.dateNaiss), "dd/MM/yyyy")
                            : ""}
                        </CardTitle>
                        <CardSubtitle className="text-muted fs-5 mt-3">
                          Phone
                    ...
                    {activeTab === "Update" && (
                      <div className="tab-pane fade show in active">
                        <div className="p-4">
                          ...
                          <Button onClick={updateDisponibilites}>
                            Update Availibility{" "}
                          </Button>
                        </div>
                      </div>
                            ...
                            <table className="result-table">
                              ...
                              <tbody>
                                {userCoursesResponse?.map((data) => (
                                  <tr key={data.id}>
                                    <td className="course">{data.name}</td>
                                    ...
                                    <td className="quizz">{data.nbrQuiz}</td>
                                    <td className="price">
                                      {data.yearlyPrice} DT
                                    </td>
                                  </tr>
                                ...
                                <tr>
                                  <td className="show-item">
                                    Displaying 1 to {userCoursesResponse.length}{" "}
                                    of {userCoursesResponse.length} courses.
                                  </td>
                                </tr>
                              </tbody>
                            </table>
